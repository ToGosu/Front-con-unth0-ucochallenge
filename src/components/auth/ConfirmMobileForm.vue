<template>
  <div class="card p-4 shadow-sm confirm-mobile-form">
    <h3 class="card-title text-center mb-4">Confirmar Número Celular</h3>
...
const successMessage = ref('Número de celular confirmado exitosamente')
...
const handleInput = (index: number, event: Event) => {
  ...
  if (isValidCode.value && index === 5) {
    // Opcional: auto-submit cuando se completa
    // handleConfirm()
  }
}
...
const handleConfirm = async () => {
  ...
  try {
    const response = await api.post(`/v1/users/confirm-mobile?code=${code}`)
    
    // Éxito
    isConfirmed.value = true
    successMessage.value = response.data?.message || 'Número de celular confirmado exitosamente'
    notifications.success(successMessage.value)
...
const handleResend = async () => {
  ...
  try {
    ...
    // await api.post('/api/v1/users/resend-mobile-code')
